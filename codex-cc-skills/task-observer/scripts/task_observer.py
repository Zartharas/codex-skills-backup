...
Observations captured during Codex task-oriented work.
...
Principles that apply across Codex skills. Read this before creating or updating skills.
...
def global_dir() -> Path:
    return codex_home() / "memories" / "task-observer"
...
def ensure_files() -> None:
    ...
    if home.exists() and not home.is_dir():
        raise SystemExit(f"CODEX_HOME must be a directory, got file: {home}")
...
def agents_paths(cwd: Path) -> List[Path]:
    home = Path.home()
    candidates = []
    for directory in [home, home / ".codex", home / ".agents"]:
...
def local_memory_dirs(cwd: Path) -> List[Path]:
    ...
    for base in dedupe([raw_cwd, resolved_cwd]):
        for parent in ancestors_root_first(base):
            candidates.append(parent / ".codex" / "memories" / "task-observer")
            candidates.append(parent / ".agents" / "memories" / "task-observer")
...
def command_context(args: argparse.Namespace) -> int:
    ...
    print()
    print("Read these AGENTS.md/agents.md files in order:")
    agent_files = agents_paths(cwd)
    if agent_files:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Codex task-observer helper")
    subparsers = parser.add_subparsers(dest="command", required=True)

    init_parser = subparsers.add_parser("init", help="Initialize global observer memory")
    init_parser.set_defaults(func=command_init)

    context_parser = subparsers.add_parser("context", help="List AGENTS.md and memory files to read")

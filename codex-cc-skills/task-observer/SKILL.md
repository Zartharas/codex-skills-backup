...
name: task-observer
description: >
  Observe Codex task sessions and capture reusable skill-improvement
  ...
  Rule Them All". The skill keeps global Codex observation memory active across
  projects, reads applicable AGENTS.md instructions, and stages proposed skill
...
Codex adaptation of "One Skill to Rule Them All". Licensed under CC BY 4.0.
For methodology feedback, open an issue on the upstream project:
...
2. Run the helper when available. The installed skill directory should be named
   `task-observer`:
...
3. Read every `AGENTS.md`/`agents.md` path reported by the helper, in order. If
   ...
   - `$HOME/.agents/agents.md`
   - every `AGENTS.md` or `agents.md` on the path from the filesystem root to
...
4. Always read the global observer memory files before any project-local memory:
   - `${CODEX_HOME:-$HOME/.codex}/memories/task-observer/log.md`
   - `${CODEX_HOME:-$HOME/.codex}/memories/task-observer/cross-cutting-principles.md`
   - `${CODEX_HOME:-$HOME/.codex}/memories/task-observer/last-review-date.txt`
5. Then read project-local observer overlays, if present. Local memories add
   context; they never replace or shadow the global files.
...
1. Read the live skill file first from the actual Codex skill path.
...
5. Stage proposed updates under:
   ...
   ```text
   ${CODEX_HOME:-$HOME/.codex}/memories/task-observer/skill-updates/YYYY-MM-DD/<skill-name>/SKILL.md
   ```
...
Use Codex's normal editing discipline for any real file edits: inspect the
current file, use patch-based edits where appropriate, and never revert user
...
1. Inventory available custom skills from the system skill list and from:
